...
def test_unverified_adding_to_favorite(eco_products_page, login_page):
    locator = f'//a[contains(text(), "{name}")]'
    eco_products_page.open()
    eco_products_page.move_attention(locator)
    eco_products_page.click_to_favorite(f'{locator}/following::a[@data-action="add-to-wishlist"]')
    assert login_page.check_unverified_action_opened('//div[@data-ui-id="message-error"]/div')
