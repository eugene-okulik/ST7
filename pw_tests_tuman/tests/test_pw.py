...
def test_create_account(create_account):
    ...
    create_account.input_met(loc.email_loc, 'tsalttr@bk.com')
...
def test_check_re_registration(create_account):
    ...
    create_account.input_met(loc.password_confirmation_loc, '923456zZz#!!')
    create_account.click_el(loc.click_button_account)
    create_account.wait_for_element_visible(loc.error_message)
...
def test_title_sale(sale_page):
    ...
    # sale_page.wait_for_element_visible(sal.title_sale)
    sale_page.check_title(sal.title_sale)
...
def test_correct_url(sale_page):
    sale_page.open()
    sale_page.current_url('https://magento.softwaretestingboard.com//sale.html')
